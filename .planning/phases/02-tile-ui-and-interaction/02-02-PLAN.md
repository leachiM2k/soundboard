...
must_haves:
  truths:
    ...
    - "Filled tiles display the recording duration as a small timestamp"
  ...
  artifacts:
    ...
    - path: "src/ui/grid.ts"
      provides: "initGrid(), updateTile(index, tile) — replaces renderTiles() pattern from main.ts"
...
Purpose: This plan creates every visual element users see — the 3x3 grid, tile appearances per state, pulse animation, and duration display. It runs parallel to Plan 01 (no shared files). Plan 03 depends on this plan's grid DOM structure existing.
...
<task type="auto">
  ...
  <action>
    **index.html** — replace current content with:
    ```html
    <!doctype html>
    <html lang="en">
      <head>
      ...
      <body>
        ...
        <dialog id="action-sheet" aria-modal="true">
          <div class="action-sheet-content">
            ...
            <button class="action-sheet-btn destructive" id="btn-delete">Löschen</button>
        ...
        <dialog id="rename-dialog" aria-modal="true">
          <form method="dialog" id="rename-form">
            ...
            <input type="text" id="rename-input" maxlength="30" placeholder="Name eingeben…" autocomplete="off" />
    ...
    Key requirements:
    - `viewport-fit=cover` in the meta viewport tag — REQUIRED for `env(safe-area-inset-bottom)` to have a non-zero value on iPhone with home bar.
    - `<div id="grid">` — the grid container (not `id="tiles"` from Phase 1).
    - Two `<dialog>` elements: `#action-sheet` and `#rename-dialog` — must be direct children of `<body>` (not inside `#app`) so they are not affected by the app's `user-select: none` CSS applied to tiles.

    **src/style.css** — create with the following CSS, implementing ALL tile states from the research:
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Filled — accent color applied via CSS custom property */
    .tile--has-sound {
    ...
    #rename-input {
      ...
      outline: none;
      margin-bottom: 14px;
      /* Do NOT set user-select: none here — input must be editable */
    }
    ...
    Note on dialog positioning: The action-sheet `<dialog>` uses `position: fixed` + `bottom: 0` with `transform: translateY(100%)` as the hidden state and `transform: translateY(0)` when `[open]`. This avoids the `display:none` transition pitfall (Pitfall 3 from research). The rename dialog uses `translate(-50%, -50%)` for centering — this overrides the browser's default `<dialog>` centering margin.
  </action>
  <verify>
    Run: `npm run build` — Vite must bundle successfully with no errors.
    Visually confirm: open `npm run dev` and check that the page shows a dark background (no broken layout). The grid container is empty at this stage (no tile elements yet — grid.ts not wired).
  </verify>
...
<task type="auto">
  ...
  <action>
    **src/ui/tile.ts** — Create with the following:
    ...
    export function buildTileContent(index: number, tile: TileData): string {
      switch (tile.state) {
        ...
        case 'has-sound': {
          ...
          // We don't have duration in SlotRecord yet — show a placeholder.
    ...
    **Important note on durationSeconds:** The current `SlotRecord` in `db.ts` does not have a `durationSeconds` field — only `recordedAt` (a Unix timestamp). Rather than attempting to derive duration from a timestamp (unreliable), the `buildTileContent` for `has-sound` should simply show the tile label without a duration. Add a TODO comment: "Duration display requires durationSeconds field on SlotRecord — add in main.ts onComplete callback." This avoids over-engineering now.
    ...
    **src/ui/grid.ts** — Create with the following:
    ...
    export function initGrid(
      ...
      state.tiles.forEach((tile, index) => {
        ...
        // Long-press handler — attached by main.ts via attachLongPress after initGrid
    ...
    Alternative (simpler): Have `initGrid` accept an optional `onLongPress` callback and let it call `attachLongPress` internally. This requires grid.ts to import long-press.ts. This is fine — do this instead:
  ...
  <verify>
    ...
    Run: `npm run build` — Vite must bundle cleanly.
