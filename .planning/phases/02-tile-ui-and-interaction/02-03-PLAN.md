...
must_haves:
  truths:
    ...
    - "Filled tiles show the recording duration as a small timestamp (e.g. '0:03') on the tile face"
  artifacts:
...
<task type="auto">
  ...
  <files>src/storage/db.ts, src/state/store.ts, src/ui/tile.ts, src/main.ts</files>
  <action>
    ...
    **src/ui/tile.ts** — update `buildTileContent` has-sound case to render duration on the tile face (locked user decision: "Filled tiles display the recording duration as a small timestamp (e.g. '0:03')"):

    In the `has-sound` branch of `buildTileContent`, after rendering the label, add the duration span:
    ```typescript
    // Inside the has-sound case, after the label element:
    if (tile.record?.durationSeconds != null) {
      const dur = document.createElement('span');
      dur.className = 'tile-duration';
      dur.textContent = formatDuration(tile.record.durationSeconds);
      content.appendChild(dur);
    }
    ```
    The `.tile-duration` CSS class already exists in `src/style.css`. `formatDuration` is already defined in `tile.ts`. This surfaces on every filled tile so users always see the recording length at a glance.

    Add `src/ui/tile.ts` to the files list for this task.
